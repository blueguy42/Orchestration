...
# Module-level default Generator — used only when callers pass rng=None.
# Isolated from the global numpy random state.
...
class Agent:
    ...
    def predict(self, task: Task, rng: np.random.Generator = None) -> int:
        """
        Simulate agent prediction based on true capability.

        Args:
            task: The task to predict.
            rng:  A numpy Generator for reproducible randomness.
                  If None, falls back to the module-level default Generator.
                  The global np.random state is never modified.
        """
        if rng is None:
            rng = _default_rng
        prob_correct = self.capabilities[task.region]
        is_correct = rng.random() < prob_correct
        return task.y if is_correct else 1 - task.y
...
class CapabilityEstimator:
    ...
    def __init__(self, K: int, M: int, alpha0: float = 1.0, alpha1: float = 1.0,
        ...
        self.counts = np.zeros((K, M, 2))  # [agent, region, 0=incorrect / 1=correct]
    ...
    def inject_estimates(self, estimates: np.ndarray, n_virtual: int = 50):
        ...
        n_virtual  : number of virtual observations per slot (default 50)
        """
        for k in range(self.K):
            for m in range(self.M):
...
class UCB1Orchestrator(BaseOrchestrator):
    ...
    agent k in region r.  Standard per-region UCB1: each region is an
    independent bandit.
    """
...
class RandomOrchestrator(BaseOrchestrator):
    """Random baseline: selects agents uniformly at random (uses isolated RNG)."""
