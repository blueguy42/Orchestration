...
Runs five experimental parts and saves all figures to output/:
  ...
  Part 1 — Baseline Orchestrators
      ...
      over N_RUNS independent seeds and reported with ±1σ bands.
  ...
  Part 3 — Unified Comparison
      ...
      absolute x-axis to visualise the efficiency advantage of teaching.

  Part 4 — Prior Sensitivity
      Re-runs machine teaching with three Beta priors (Uniform, Jeffreys,
      Expert) on the Varying scenario.  Reports MSE curves, efficiency bars,
      and a final-MSE heatmap (teachers × priors).
  ...
  Part 5 — Budget vs Downstream Accuracy
      ...
      with Oracle and Random baselines for reference.
"""
...
FIGSIZE = (13, 6)
BAND_ALPHA = 0.15  # Transparency for confidence bands
...
def run_baseline_experiments() -> dict:
    ...
    for sc_name, agent_factory in SCENARIOS.items():
        ...
        for run in range(N_RUNS):
            if run and run % 25 == 0: print(f"    run {run}")
            gen = SyntheticDataGenerator(M=M, seed=SEED + run)
            tasks = gen.generate_task_stream(N_TASKS)
            res = run_all_baselines(agents, tasks, verbose=False)
            if not avg:
                for k in res: avg[k] = {"overall_accuracy": 0.0}
            for k, v in res.items():
                avg[k]["overall_accuracy"] += v["overall_accuracy"]
        for k in avg: avg[k]["overall_accuracy"] /= N_RUNS
        avg["__app__"] = app
...
def plot_baseline_accuracy(all_results: dict, output_path: str = None):
    ...
    x = np.arange(len(scenarios)); n = len(orch_names); w = 0.14
    ...
    for i, name in enumerate(orch_names):
        ...
        ax.bar(x + (i - n/2 + 0.5)*w, accs, w,
               ...
               color=BASELINE_COLORS.get(name, "grey"), alpha=0.88, edgecolor="white")
    _bar_labels(ax, fmt="{:.3f}", fontsize=7.5)
    ax.set_xticks(x); ax.set_xticklabels(scenarios, fontsize=10.5)
    ax.set_ylabel("Overall Accuracy")
    ax.set_title("Baseline Orchestrators — Accuracy by Scenario", fontsize=13, fontweight="bold")
    ax.legend(fontsize=9, loc="lower right"); ax.set_ylim(0, 1.12)
    plt.tight_layout()
    if output_path: _save(fig, output_path)
    return fig


def compute_baseline_curves(window: int = 50) -> dict:
    """Returns {scenario: {orch: {"acc_runs": 2d, "mse_runs": 2d}}}"""
    orch_defs = [
    ...
    for sc_name in SCENARIOS:
        ...
        for cls, kwargs, key in orch_defs:
            acc_runs, mse_runs = [], []
            for run in range(N_RUNS):
                if run and run % 25 == 0: print(f"    {key} run {run}")
                gen = SyntheticDataGenerator(M=M, seed=SEED + run)
                tasks = gen.generate_task_stream(N_TASKS)
            ...
            sc[key] = {
                "acc_runs": np.array(acc_runs),   # (N_RUNS, N_TASKS-window)
                "mse_runs": np.array(mse_runs),   # (N_RUNS, N_TASKS)
            }
...
def plot_baseline_learning_curves(all_results: dict, output_path: str = None,
    ...
    """4-panel rolling accuracy with ±1σ bands."""
    scenarios = list(all_results.keys())
    ...
    for ax, sc in zip(axes, scenarios):
        ...
        ax.set_xlabel("Task Index"); ax.set_ylabel(f"Rolling Accuracy (w={window})")
        ax.legend(fontsize=7)
    fig.suptitle("Baseline Orchestrators — Learning Curves (±1σ)",
...
def plot_baseline_capability_mse(baseline_curves: dict, output_path: str = None):
    ...
    for ax, sc in zip(axes, scenarios):
        for key in orch_keys:
            ...
            ax.fill_between(xs, mean - std, mean + std,
                            color=BASELINE_COLORS[key], alpha=BAND_ALPHA)
        ax.axhline(TARGET_MSE, color="grey", ls="--", lw=1.2, label=f"Target={TARGET_MSE}")
        ax.set_title(sc, fontsize=11, fontweight="bold")
        ax.set_xlabel("Task Index"); ax.set_ylabel("Capability MSE")
        ax.legend(fontsize=7)
    fig.suptitle("Baseline Orchestrators — Capability MSE (±1σ)",
                 fontsize=13, fontweight="bold", y=1.01)
    plt.tight_layout()
    if output_path: _save(fig, output_path)
    return fig
...
def run_teaching_experiments() -> dict:
    """Run all teachers × all scenarios, storing per-run curves for bands."""
    print("\n" + "=" * 70)
    ...
    for sc_name, agent_factory in SCENARIOS.items():
        ...
        for run in range(N_RUNS):
            ...
            for name, summary in res.items():
                ...
                sc_data[name]["final_est_runs"].append(summary["final_estimates"].copy())

        # Compute averaged summary + keep raw runs for bands
        for name in sc_data:
            d = sc_data[name]
        ...
        # Print summary
        avg_dict = {n: {"final_mse": sc_data[n]["final_mse"],
...
def plot_mse_curves(all_results: dict, output_path: str = None):
    """4-panel MSE curves with ±1σ bands."""
    scenarios = list(all_results.keys())
    fig, axes = plt.subplots(2, 2, figsize=(14, 9)); axes = axes.flatten()
    for ax, sc in zip(axes, scenarios):
        for name in all_results[sc]:
            d = all_results[sc][name]
            arr = d["mse_runs"]  # (N_RUNS, BUDGET)
            mean = arr.mean(axis=0); std = arr.std(axis=0)
            xs = np.arange(len(mean))
            ax.plot(xs, mean, color=TEACHER_COLORS.get(name, "black"),
                    label=TEACHER_LABELS.get(name, name), linewidth=2)
            ax.fill_between(xs, mean - std, mean + std,
                            color=TEACHER_COLORS.get(name, "black"), alpha=BAND_ALPHA)
        ax.axhline(TARGET_MSE, color="grey", ls="--", lw=1.2, label=f"Target={TARGET_MSE}")
        ax.set_title(sc, fontsize=11, fontweight="bold")
        ax.set_xlabel("Evaluation Steps"); ax.set_ylabel("MSE")
        ax.legend(fontsize=7)
    fig.suptitle("Machine Teaching — MSE Curves (±1σ)",
                 fontsize=13, fontweight="bold", y=1.01)
    plt.tight_layout()
    if output_path: _save(fig, output_path)
    return fig
...
def plot_efficiency_bars(all_results: dict, output_path: str = None):
    ...
    n_t = len(teacher_names); x = np.arange(len(scenarios)); w = 0.15
    ...
    for i, name in enumerate(teacher_names):
        ...
        ax.bar(x + (i - n_t/2 + 0.5)*w, steps, w,
               ...
               color=TEACHER_COLORS.get(name, "grey"), alpha=0.87, edgecolor="white")
    ...
    ax.set_title("Teaching Efficiency — Steps to Target MSE", fontsize=13, fontweight="bold")
    ax.legend(fontsize=9); ax.set_ylim(0, BUDGET * 1.18)
    plt.tight_layout()
    if output_path: _save(fig, output_path)
    return fig
...
def plot_final_mse_heatmap(all_results: dict, output_path: str = None):
    ...
    ax.set_yticks(range(len(scenarios))); ax.set_yticklabels(scenarios, fontsize=10)
...
def plot_capability_estimates(all_results: dict, scenario: str = "Varying",
    ...
    for k in range(K):
        for m in range(Ml):
            ax = axes[k, m]; truth = true_caps[k, m]
            ...
            ax.axhline(truth, color="black", ls="--", lw=1.5, label=f"True={truth:.3f}")
            ax.set_xticks(range(n_t))
            ax.set_xticklabels(lbls, rotation=30, ha="right", fontsize=8)
            ax.set_ylim(-0.05, 1.05); ax.set_ylabel("P(correct)")
...
def plot_convergence_speed(all_results: dict, output_path: str = None):
    ...
    for i, (thr, lbl, col) in enumerate(zip(thresholds, labels, colors)):
        ...
        ax.bar(x + (i-1)*w, steps, w, label=lbl, color=col, alpha=0.88, edgecolor="white")
    _bar_labels(ax, fmt="{:.0f}", fontsize=7.5)
    ax.set_xticks(x)
    ax.set_xticklabels([TEACHER_LABELS.get(n, n) for n in teacher_names], fontsize=10)
    ax.set_ylabel("Steps until threshold"); ax.set_ylim(0, BUDGET*1.18)
...
def plot_mse_auc(all_results: dict, output_path: str = None):
    ...
    fig, ax = plt.subplots(figsize=FIGSIZE)
    for i, name in enumerate(teacher_names):
        aucs = [float(np.trapezoid(all_results[sc][name]["mse_curve"])) for sc in scenarios]
        ax.bar(x + (i - n_t/2 + 0.5)*w, aucs, w,
               label=TEACHER_LABELS.get(name, name),
               color=TEACHER_COLORS.get(name, "grey"), alpha=0.87, edgecolor="white")
    _bar_labels(ax, fmt="{:.1f}", fontsize=7.5)
    ax.set_xticks(x); ax.set_xticklabels(scenarios, fontsize=10)
    ax.set_ylabel("AUC (lower = faster)"); ax.legend(fontsize=9)
...
def evaluate_downstream_orchestration(teaching_results: dict, n_tasks: int = 500):
    """Per-run downstream eval (re-runs teaching to get per-run estimates)."""
    print("\n" + "=" * 70)
    ...
    for run in range(N_RUNS):
        if run and run % 25 == 0: print(f"  run {run}")
        tr = run_teaching_experiment(agents=agents, M=M, budget=BUDGET,
                                     teacher_classes=TEACHER_CLASSES, seed=SEED + run)
    ...
    for k in dt_avg: dt_avg[k] /= N_RUNS
    for k in bl_avg: bl_avg[k] /= N_RUNS
    print(f"\n{'Method':<35} {'Accuracy':>8}")
    print("-" * 45)
    for k, v in bl_avg.items():  print(f"  [Baseline] {BASELINE_LABELS[k]:<22} {v:.3f}")
    for k, v in dt_avg.items():  print(f"  [Teaching] {TEACHER_LABELS[k]:<22} {v:.3f}")
...
def plot_unified_accuracy(bl: dict, dt: dict, output_path: str = None):
    ...
    lbls = [BASELINE_LABELS[k] for k in bl] + [""] + [TEACHER_LABELS[k] for k in dt]
    cols = [BASELINE_COLORS[k] for k in bl] + ["none"] + [TEACHER_COLORS[k] for k in dt]
    ...
    ax.set_xticks(xs); ax.set_xticklabels(lbls, rotation=18, ha="right", fontsize=10)
    ax.set_ylabel("Downstream Accuracy"); ax.set_ylim(0, 1.12)
    ax.set_title("Unified Accuracy — Varying Scenario", fontsize=12, fontweight="bold")
    legend_elems = [
        Line2D([0],[0], marker="s", color="w", markerfacecolor="#4CAF50", markersize=10, label="Baselines"),
        Line2D([0],[0], marker="s", color="w", markerfacecolor="#2196F3", markersize=10, label="Teaching-guided"),
    ]
...
def plot_convergence_mse_absolute(baseline_curves: dict, teaching_results: dict,
                                  output_path: str = None):
    """
    4-panel: capability MSE on ABSOLUTE x-axis (evaluation steps).
    Baselines run 0→1000; teaching runs 0→200.  The efficiency gap is
    visually obvious because teaching curves stop at step 200.
    """
    scenarios = list(SCENARIOS.keys())
    orch_keys = ["RandomOrchestrator","GreedyOrchestrator",
                 "UCB1Orchestrator","PaperOrchestrator"]
    ...
    for ax, sc in zip(axes, scenarios):
        ...
        ax.axhline(TARGET_MSE, color="grey", ls=":", lw=1.2, label=f"Target={TARGET_MSE}")
        ax.axvline(BUDGET, color="grey", ls="-.", lw=1, alpha=0.5, label=f"Budget={BUDGET}")
        ax.set_title(sc, fontsize=11, fontweight="bold")
        ax.set_xlabel("Evaluation Steps (absolute)")
        ax.set_ylabel("Capability MSE"); ax.legend(fontsize=5.5, ncol=2)
    style_legend = [
        Line2D([0],[0], color="black", lw=2, ls="-", label="Baselines (over task stream)"),
        Line2D([0],[0], color="black", lw=2, ls="--", label="Teaching (over budget)"),
    ]
    fig.legend(handles=style_legend, fontsize=9, loc="lower center",
               ncol=2, bbox_to_anchor=(0.5, -0.02))
    fig.suptitle("Convergence MSE — Absolute Axis (±1σ)", fontsize=13, fontweight="bold", y=1.01)
    plt.tight_layout()
    if output_path: _save(fig, output_path)
    return fig
        ...
        for run in range(N_RUNS):
            ...
            for name, summary in res.items():
                if name not in pr_data:
                    pr_data[name] = {"mse_runs": [], "final_mse_runs": [],
                ...
                pr_data[name]["final_est_runs"].append(summary["final_estimates"].copy())
        for name in pr_data:
            d = pr_data[name]
...
def plot_prior_mse_curves(prior_results: dict, scenario_name: str = "Varying",
                          output_path: str = None):
    """3 rows × 1 col, MSE curves per prior with ±1σ bands."""
    ...
    for ax, pn in zip(axes, priors_list):
        for tname in teacher_names:
            ...
            ax.fill_between(xs, mean-std, mean+std,
                            color=TEACHER_COLORS.get(tname, "black"), alpha=BAND_ALPHA)
        ...
        ax.set_ylabel("MSE"); ax.legend(fontsize=8, loc="upper right")
...
def plot_prior_efficiency_bars(prior_results: dict, scenario_name: str = "Varying",
    ...
    priors_list = list(prior_results.keys())
    teacher_names = [cls.__name__ for cls, _ in TEACHER_CLASSES]
    n_p = len(priors_list); x = np.arange(len(teacher_names)); w = 0.25
    fig, ax = plt.subplots(figsize=FIGSIZE)
    for i, pn in enumerate(priors_list):
        ...
        ax.bar(x + (i - n_p/2 + 0.5)*w, steps, w,
               label=PRIOR_LABELS.get(pn, pn), color=PRIOR_COLORS.get(pn, "grey"),
               alpha=0.85, edgecolor="white")
    _bar_labels(ax, fmt="{:.0f}", fontsize=7.5)
    ax.axhline(BUDGET, color="grey", ls="--", lw=1, label="Budget limit")
    ax.set_xticks(x)
    ax.set_xticklabels([TEACHER_LABELS.get(n,n) for n in teacher_names], fontsize=10)
    ax.set_ylabel(f"Steps to MSE < {TARGET_MSE}"); ax.set_ylim(0, BUDGET*1.18)
...
def plot_prior_final_mse_heatmap(prior_results: dict, scenario_name: str = "Varying",
    ...
    data = np.array([[prior_results[p].get(tn, {}).get("final_mse", np.nan)
                      for p in priors_list] for tn in teacher_names])
    ...
    ax.set_title(f"Prior Sensitivity — Final MSE  |  {scenario_name}", fontsize=12, fontweight="bold")
...
# ══════════════════════════════════════════════════════════════════════════════
# 5. BUDGET vs DOWNSTREAM ACCURACY TRADE-OFF  (NEW)
# ══════════════════════════════════════════════════════════════════════════════

def run_budget_tradeoff(budget_values=None, n_downstream: int = 500) -> dict:
    """
    For each teaching budget in budget_values, run each teacher on the
    Varying scenario, then evaluate downstream accuracy with PaperOrchestrator.
    ...
    results = {tn: {"budgets": budget_values, "acc_runs": {b: [] for b in budget_values}}
               for tn in teacher_names}
    ...
    # Print table
    print(f"\n{'Teacher':<25}", end="")
    for b in budget_values: print(f"  B={b:>3}", end="")
    print()
    print("-" * (25 + 7 * len(budget_values)))
    for tn in teacher_names:
        print(f"{TEACHER_LABELS[tn]:<25}", end="")
        for m in results[tn]["acc_mean"]: print(f"  {m:.3f}", end="")
        print()
...
def plot_budget_tradeoff(tradeoff_results: dict, output_path: str = None):
    """
    Line plot: x = teaching budget, y = downstream accuracy.
    One line per teacher with ±1σ bands.
    Also shows Oracle & Random baselines as horizontal dashed lines.
    """
    fig, ax = plt.subplots(figsize=FIGSIZE)
    ...
    for tname in teacher_names:
        ...
        ax.plot(budgets, means, color=TEACHER_COLORS[tname],
                label=TEACHER_LABELS[tname], linewidth=2.5, marker="o", markersize=5)
    ...
    for run in range(N_RUNS):
        ...
        for cls, key, acc_list in [(OracleOrchestrator, "oracle", oracle_accs),
                                    (RandomOrchestrator, "random", random_accs)]:
            tc = [Task(x=t.x.copy(), y=t.y, region=t.region) for t in tasks]
            orch = cls(agents, M)
            for t, task in enumerate(tc):
                aidx = orch.select_agent(task, t)
...
def main():
    np.random.seed(SEED)
    print("\n" + "=" * 70)
    print(f" Multi-Agent Orchestration — Unified Experiment Suite")
    ...
    plot_baseline_learning_curves(baseline_results, "output/1b_baseline_learning_curves.png",
                                  curve_data=baseline_curves)
    plot_baseline_capability_mse(baseline_curves, "output/1c_baseline_capability_mse.png")
    ...
    # Part 3 — Unified comparison
    bl_accs, dt_accs = evaluate_downstream_orchestration(teaching_results)
    plot_unified_accuracy(bl_accs, dt_accs, "output/3a_unified_accuracy_comparison.png")
    ...
    plot_prior_mse_curves(prior_results, "Varying", "output/4a_prior_mse_curves.png")
    plot_prior_efficiency_bars(prior_results, "Varying", "output/4b_prior_efficiency_bars.png")
    plot_prior_final_mse_heatmap(prior_results, "Varying", "output/4c_prior_final_mse_heatmap.png")

    # Part 5 — Budget vs Downstream Accuracy
    tradeoff_results = run_budget_tradeoff()
    plot_budget_tradeoff(tradeoff_results, "output/5a_budget_vs_downstream_accuracy.png")
